...
func TestEtcdClusterUpgrade(t *testing.T) {
	// TODO: remove once the per-member Recreating workflow is implemented
	// (issue #470 follow-up). Until then the operator marks a member for
	// upgrade but never actually recreates its Pod, so this test would
	// time out waiting for every member to reach CurrentVersion == toVersion.
	t.Skip("Recreating workflow not yet implemented; bump Spec.Version never rolls the Pod")
	...
	testCases := []struct {
		...
		// TODO: add 3.5.x -> 3.6.x and 3.6.x -> 3.7.x once those paths are supported
	}
