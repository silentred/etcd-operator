...
func getLeaderPodName(t *testing.T, c *envconf.Config, clusterName string) (string, error) {
	...
	if len(endpoints) != 1 || endpoints[0].Status.Leader == 0 {
	...
	for _, member := range members.Members {
		if member.ID == endpoints[0].Status.Leader && member.Name != "" {
			return member.Name, nil
		}
	}
	return "", fmt.Errorf("leader %x not found in named etcd members", endpoints[0].Status.Leader)
}
